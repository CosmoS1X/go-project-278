...
func TestRepositoryRecordVisit(t *testing.T) {
	...
	created, err := repo.Create(t.Context(), "https://example.com", "vis1")
	require.NoError(t, err)

	err = repo.RecordVisit(t.Context(), created.ID, "https://google.com", "1.2.3.4", "Mozilla/5.0", 302)
	require.NoError(t, err)
	...
	for _, v := range visits {
		if v.LinkID != created.ID || v.IP != "1.2.3.4" {
			continue
		}
	...
	assert.True(t, found, "expected visit with link_id=%d and ip=1.2.3.4", created.ID)
}

func TestRepositoryListVisits(t *testing.T) {
	repo := newTestRepository(t)

	created, err := repo.Create(t.Context(), "https://example.com", "lst1")
	require.NoError(t, err)

	require.NoError(t, repo.RecordVisit(t.Context(), created.ID, "https://a.com", "10.0.0.1", "Chrome", 302))
	require.NoError(t, repo.RecordVisit(t.Context(), created.ID, "https://b.com", "10.0.0.2", "Firefox", 302))
